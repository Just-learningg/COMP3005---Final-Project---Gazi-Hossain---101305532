...
CREATE VIEW SessionsForEachMember AS
    SELECT s.s_date,
           s.start_time,
           s.end_time,
           t.trainerID,
...
SELECT * 
FROM Member m
JOIN Contact c ON m.memberID = c.memberID
JOIN PersonalFitnessData p ON m.memberID = p.memberID;
--test 2
SELECT * 
FROM Member m
JOIN membersubscriptions ms ON m.memberID = ms.memberID
JOIN paymentrecords pr on ms.subscriptionID = pr.subscriptionID;
--test 3
SELECT * FROM  trainerptsessionsview;
